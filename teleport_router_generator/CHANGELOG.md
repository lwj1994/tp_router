## 0.8.2
* **Sync**: Synchronize version with `teleport_router` 0.8.2.
...
## 0.6.0
* **Fix**: Removed redundant `TeleportRouteObserver` generation (now handled by `TeleportNavKey` logic).
...
*   **🔄 Support for Class-based Callbacks**: Updated generator to instantiate and use `TeleportRedirect` and `TeleportOnExit` classes.
*   **🛠️ Type Reconstruction**: Added generation of `static T fromData(TeleportRouteData)` for all route classes.
